...
export default function CustomerQueriesPage() {
  ...
  const saveChanges = async () => {
    ...
    const statusChanged = selected.status !== updated.status;
    const previousResponse = (selected.admin_note ?? "").trim();
    const updatedResponse = (updated.admin_note ?? "").trim();
    const responseChanged = previousResponse !== updatedResponse;
     ...
     * =========================================================
     *
     * Database update always happens first.
     *
     * Email delivery is intentionally non-blocking for the
     * ticket operation: if Resend/email delivery fails, the
     * ticket remains successfully saved.
     *
     * Email rules:
     *
     * - New → In Progress
     ...
     *     closed
     *
     * - Same status + Support Response changed
     *     response_added
     *
     * This also prevents duplicate emails when the admin opens
     * a ticket and clicks Save without actually changing it.
     */
    try {
      let emailEvent:
        | "status_changed"
      ...
      if (statusChanged) {
        if (updated.status === "resolved") {
          ...
          emailEvent = "status_changed";
        }
      } else if (responseChanged && updatedResponse) {
        emailEvent = "response_added";
      }

      if (emailEvent && updated.ticket_number && updated.email) {
        await notificationService.sendCustomerQueryEmail({
          to: updated.email,
      ...
      console.error(
        "Ticket email notification failed. Ticket was saved successfully:",
        emailError
      );
    }
