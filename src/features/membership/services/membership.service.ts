...
class MembershipService {
  ...
  async getMembers(): Promise<Membership[]> {
    ...
    return (data ?? []).map((item: any): Membership => {
      const currentSpend = Number(
        item.lifetime_spend ?? 0
      );

      const currentTierIndex =
        tiers?.findIndex(
          (tier) => tier.id === item.tier?.id
        ) ?? -1;
  ...
  async getMembershipStats() {
    ...
    return {
      ...
      totalRewardPoints: members.reduce(
        (sum, member) =>
          sum + member.current_points,
        0
      ),

      totalLifetimeSpend: members.reduce(
        (sum, member) =>
  ...
  async getMember(customerId: string): Promise<Membership> {
